...
def build(facts) -> Section:
    ...
    if comparable_from:
        detail.append(
            ...
            "onward is comparable; earlier weeks are shown but excluded "
            "from the median."
        )
...
def series(facts, *, weeks: Optional[int] = None) -> list[WeekPoint]:
    ...
    for window in windows:
        ...
        week_hash = _hash_in(facts, window)
        points.append(
            WeekPoint(
                label=window.label,
                ...
                settings_hash=week_hash,
                # A week with no observations can't disagree with the current
                # definition, so it stays comparable rather than being
                # excluded for missing metadata.
                comparable=week_hash is None or week_hash == latest_hash,
            )
        )
    return points
