...
class TaskInfo:
    ...
    # Recurrence provenance. A recurring instance's dates are generated by
    # Taskwarrior, so reviews must not read them as decisions someone made.
    recur: Optional[str] = None  # the template's period, e.g. "weekly"
    parent: Optional[str] = None  # uuid of the template this came from

    @property
    def is_recurring(self) -> bool:
        """True for a recurrence template or one of its instances.

        Automated changes must not be attributed to personal behaviour
        without evidence: a weekly chore that is perpetually open is doing
        exactly what it was set up to do, and counting it as a stagnant task
        would fill the triage list with the things working correctly.
        """
        return bool(self.recur or self.parent)
